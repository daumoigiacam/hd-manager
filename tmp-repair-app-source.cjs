...
const targetPath = 'D:/quản lý bán hàng 1/src/App.jsx';
...
function corrupt(str, enc) {
  ...
  return enc === 'latin1' ? buf.toString('latin1') : iconv.decode(buf, enc);
}
...
const manualMap = new Map(Object.entries({
  ...
  'ÄÄƒng nháº­p':'Đăng nhập','Táº¡o CĂ´ng ty':'Tạo Công ty','ÄÄƒng Nháº­p':'Đăng Nhập',
  'DĂ nh cho chá»§ doanh nghiá»‡p vĂ  cĂ¡c tĂ i khoáº£n bá»™ pháº­n':'Dành cho chủ doanh nghiệp và các tài khoản bộ phận',
  'VĂ o á»¨ng Dá»¥ng':'Vào Ứng Dụng','TĂ i khoáº£n demo cĂ³ sáºµn':'Tài khoản demo có sẵn',
  'Báº±ng viá»‡c Ä‘Äƒng nháº­p, báº¡n Ä‘á»“ng Ă½ vá»›i ChĂ­nh sĂ¡ch Báº£o máº­t. Dá»¯ liá»‡u Ä‘Æ°á»£c mĂ£ hĂ³a trĂªn Cloud.':'Bằng việc đăng nhập, bạn đồng ý với Chính sách Bảo mật. Dữ liệu được mã hóa trên Cloud.',
  'VĂ ':'Và','vĂ ':'và'
}));
...
function repair(input) {
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    for (const key of keys) {
      out = out.split(key).join(manualMap.get(key) || autoMap.get(key));
    }
    if (out === before) break;
  }
  return out;
...
fs.writeFileSync(targetPath, repaired, 'utf8');
