...
const errorEnvelope = (
  message,
  { status = 400, code = "REQUEST_REJECTED" } = {},
) =>
  new Response(
    JSON.stringify({
      success: false,
...
test("uses the operator finance history contract without substituting customer-portal payments or caller tenant input", async () => {
        ...
        items: [
          {
            ...
            amount: "12.50",
            metadata: {
              __hdcoProjection: { sourceRecordId: "legacy-payment" },
  ...
  const page = await api.listPaymentHistory({
    page: 1,
    limit: 100,
    companyId: "forged",
    tenantId: "forged",
  ...
  assert.equal(
    calls[1].path,
    "/finance-suite/customers/native-customer/historical-ledger",
  );
  await assert.rejects(() => api.getHistoricalCustomerLedger(""), {
...
test("keeps a customer VPS session and routes bank links without caller tenant or customer IDs", async () => {
  ...
  const api = createHdConnectStagingApi({
    get: async (path, options) => {
      ...
      return [
        { id: "bank-1", customerId: "customer-1", companyId: "company-1" },
      ];
    },
...
test("routes customer-support chat through scoped VPS contracts without caller tenant or customer IDs", async () => {
  ...
  const chats = await api.listCustomerSupportChats({
    companyId: "attacker-company",
  });
  const opened = await api.createCustomerSupportChat(
    "8b9bcebd-0f5a-4920-98b1-7be3afc37c5b",
  );
  ...
  assert.equal(
    normalizeVpsCustomerSupportMessage({ senderType: "PORTAL_USER" }).type,
...
test("fails closed when inventory mapping or unit evidence is incomplete", async () => {
  assert.throws(
    ...
    (error) =>
      error instanceof HdApiError &&
      error.code === "INVENTORY_TARGET_MAPPING_REQUIRED",
  );
  assert.throws(
    () =>
      buildVpsInventoryTransaction({
    ...
    (error) =>
      error instanceof HdApiError &&
      error.code === "INVENTORY_UNIT_MAPPING_MISMATCH",
  );
});
...
test("isolates production VPS tokens from staging browser sessions", () => {
  ...
  assert.equal(
    storage.getItem("hdconnect.vps-production.access-token"),
    "production-access",
  );
  assert.equal(
    storage.getItem("hdconnect.vps-production.refresh-token"),
    "production-refresh",
  );
...
test("refreshes once and retries an authenticated read after HTTP 401", async () => {
  ...
  const client = new HdApiClient({
    ...
    fetchImpl: async (url, init) => {
      ...
      if (
        calls.filter((call) => call.url.endsWith("/master-data/customers"))
          .length === 1
      ) {
        return errorEnvelope("Access token expired.", {
          status: 401,
          code: "UNAUTHORIZED",
        });
      }
...
test("parses authenticated VPS SSE events without exposing the access token", async () => {
  ...
  await client.stream("/realtime/stream", {
  ...
  assert.equal(
    requests[0].url,
    "https://staging-api.example.test/api/v1/realtime/stream",
  );
...
test("rejects malformed API envelopes without exposing sensitive request data", async () => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof HdApiError && error.code === "API_REQUEST_REJECTED",
  );
});
...
test("blocks order creation until the target warehouse and line-unit mapping are proven", async () => {
  const api = createHdConnectStagingApi({
    post: async () => {
      throw new Error(
        "The client must not call the API for unresolved orders.",
      );
    },
  });
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof HdApiError &&
      error.code === "ORDER_WAREHOUSE_UNRESOLVED",
  );
});
...
test("sends order line updates through the VPS sales contract", async () => {
  ...
  assert.equal(
    capturedPath,
    "/sales/orders/33333333-3333-4333-8333-333333333333",
  );
...
test("does not retry a failed POST mutation automatically", async () => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof HdApiError && error.code === "API_NETWORK_ERROR",
  );
...
test("customer read maps only typed HR assignment to UI aliases and keeps login owner in a separate read field", () => {
  ...
  const ownerOnly = normalizeVpsCustomer({
    ...customerHrFixture,
...
test("customer POST serializes salesEmpId and actual App empId as salesEmployeeId, never as a login owner", async () => {
  ...
  for (const key of ["empId", "salesEmpId", "salesEmployeeId"]) {
    const saved = await api.createCustomer({
      name: "Customer",
      [key]: customerHrFixture.salesEmployeeId,
      clientMutationId: "stable-customer-hr",
    });
...
test("customer PATCH recognizes one edited UI alias without reusing stale normalized aliases or rewriting User ownership", async () => {
  ...
  for (const field of ["empId", "salesEmpId", "salesEmployeeId"]) {
    const saved = await api.updateCustomer(customerHrFixture.id, {
      ...normalizeVpsCustomer(customerHrFixture),
      [field]: newEmployee,
      clientMutationId: "same-retry",
    });
    assert.equal(
      calls.at(-1).path,
      `/master-data/customers/${customerHrFixture.id}`,
    );
...
test("customer assignments distinguish omitted HR fields, explicit unassignment and the existing User owner contract", async () => {
  ...
  const api = createHdConnectStagingApi({
    post: async (_path, payload) => {
      body = payload;
      return { ...customerHrFixture, ...payload };
    },
  });
  await api.createCustomer({
    name: "No HR assignment",
    salesOwnerId: customerHrFixture.salesOwnerId,
  });
  assert.equal(body.salesOwnerId, customerHrFixture.salesOwnerId);
  assert.equal(Object.hasOwn(body, "salesEmployeeId"), false);
  await api.createCustomer({
    name: "Both separate targets",
    salesOwnerId: customerHrFixture.salesOwnerId,
    salesEmpId: customerHrFixture.salesEmployeeId,
  });
  assert.equal(body.salesOwnerId, customerHrFixture.salesOwnerId);
  assert.equal(body.salesEmployeeId, customerHrFixture.salesEmployeeId);
  await api.createCustomer({
...
test("customer assignment rejects non-UUID, ambiguous and unmapped historical HR references before transport", async () => {
  const api = createHdConnectStagingApi({
    post: () => assert.fail("Invalid HR mapping must not reach transport"),
    patch: () => assert.fail("Invalid HR mapping must not reach transport"),
  });
  for (const value of [
    "legacy-employee",
    "undefined",
    ...
    12,
    [],
    [customerHrFixture.salesEmployeeId],
    { id: customerHrFixture.salesEmployeeId },
  ]) {
    await assert.rejects(
      api.createCustomer({ name: "Invalid assignment", salesEmpId: value }),
  ...
  await assert.rejects(
    api.createCustomer({
      name: "Ambiguous",
      empId: customerHrFixture.salesEmployeeId,
  ...
  for (const attributes of [
    ...
    { salesEmpId: customerHrFixture.salesOwnerId },
    {
      salesEmployeeId: customerHrFixture.salesEmployeeId,
      legacyUi: { empId: "legacy-employee" },
    },
  ]) {
    const historical = normalizeVpsCustomer({
      ...customerHrFixture,
      salesEmployeeId: null,
      attributes,
    });
    ...
    await assert.rejects(api.updateCustomer(historical.id, historical), {
      code: "reconciliation_required",
    });
  }
});
...
test("explicit native HR selection reconciles a historical assignment without discarding the raw source reference", async () => {
  ...
  const saved = await api.updateCustomer(historical.id, {
    ...historical,
    empId: customerHrFixture.salesEmployeeId,
  });
  assert.equal(body.salesEmployeeId, customerHrFixture.salesEmployeeId);
  ...
  const api = createHdConnectStagingApi(
    new HdApiClient({
      ...
      fetchImpl: async (_url, init) => {
        ...
        const body = JSON.parse(init.body);
        assert.equal(
          body.salesEmployeeId,
          "33333333-3333-4333-8333-333333333333",
        );
        assert.equal(Object.hasOwn(body, "salesOwnerId"), false);
        return errorEnvelope(
          "Assignment conflicts with existing User ownership.",
          { status: 409, code: "CUSTOMER_SALES_ASSIGNMENT_CONFLICT" },
        );
      },
    }),
  );
  await assert.rejects(
    api.updateCustomer(current.id, {
      ...current,
      empId: "33333333-3333-4333-8333-333333333333",
    }),
    { code: "CUSTOMER_SALES_ASSIGNMENT_CONFLICT" },
  );
...
test("routes VPS identity security contracts without Firebase tokens", async () => {
  ...
  const api = createHdConnectStagingApi({
    get: async (path, options) => {
      ...
      if (path === "/audit")
        return { items: [{ id: "audit-1", action: "AUTH_LOGIN_SUCCEEDED" }] };
      return { id: "user-1" };
    },
    post: async (path, payload, options) => {
      calls.push({ method: "POST", path, payload, options });
  ...
  await api.changeIdentityPassword({
...
test("maps VPS sessions to the Identity Security Center and blocks unsupported operations explicitly", async () => {
  ...
  assert.equal(
    (await securityApi.identityListDevices()).devices[0].deviceId,
    "22222222-2222-4222-8222-222222222222",
  );
  await securityApi.identityRevokeDevices({
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof HdApiError &&
      error.code === "VPS_IDENTITY_DELETE_NOT_READY" &&
      error.status === 501,
  );
  await assert.rejects(
    ...
    (error) =>
      error instanceof HdApiError &&
      error.code === "VPS_IDENTITY_BIOMETRIC_NOT_READY",
  );
});
...
test("rejects invalid VPS identity mutation input before transport", async () => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof HdApiError &&
      error.code === "IDENTITY_SESSION_ID_INVALID",
  );
...
test("routes supported warehouse, inventory, finance and HR contracts without tenant override", async () => {
  ...
  await api.createFinanceCashAccount({
    companyId: "attacker-company",
    code: "CASH-A",
    name: "Cash",
  });
    ...
    lines: [
      {
        ...
        unitId: "11111111-1111-4111-8111-111111111115",
        quantity: 1,
      },
    ],
...
test("routes VPS stock count as an explicit session, line, and post sequence", async () => {
  ...
  const api = createHdConnectStagingApi({
    post: async (path, payload, options) => {
      ...
      if (path === "/warehouse-suite/counts")
...
test("does not retry a forbidden VPS realtime stream", async () => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof HdApiError && error.code === "AUTH_PERMISSION_DENIED",
  );
...
test("routes tenant-scoped legacy business history without accepting a caller tenant override", async () => {
  ...
  const api = createHdConnectStagingApi({
    get: async (path, options) => {
      calls.push({ path, options });
      if (path === "/legacy-business/summary")
        return { total: 4338, domains: {} };
      return {
        items: [{ id: "legacy-1", companyId: "company-1" }],
        pagination: { totalItems: 1 },
      };
    },
  });
